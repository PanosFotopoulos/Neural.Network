...
for dense_layer in dense_layers:
    for layer_size in layer_sizes:
        for conv_layer in conv_layers:
            NAME = '{}-conv-{}-nodes-{}-ONLY-OUTPUT-dense-{}'.format(conv_layer, layer_size, dense_layers, int(time.time()))
            tensorboard = TensorBoard(log_dir='./logs/{}'.format(NAME))
            # Model setup
            model = Sequential()
            ...
            # Train the model
            model.fit(X, np.array(y), batch_size=32, epochs=10, validation_split=0.1,callbacks=[tensorboard])#tensorboard --logdir='logs/'

model.save(r'C:\Users\Panos\Desktop\PythonProject\n_n\Final_Models\3x64NODENSE-CNN1.keras')
